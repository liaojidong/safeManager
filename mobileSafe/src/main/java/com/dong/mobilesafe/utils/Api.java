...
public final class Api {
     ...
     * @param msg message
     */
	public static void alert(final Context ctx, final CharSequence msg) {
		...
		mainHandler.post(new Runnable() {
			...
			public void run() {
		    	if (ctx != null) {
					DialogUtils.showTipDialog(ctx,msg.toString(), DialogPlus.Gravity.CENTER);
		    	}
	...
	public static DroidApp[] getApps(Context ctx) {
		...
		try {
			...
			DroidApp app = null;
			for (final PackageInfo packInfo : appInfos) {
				final int uid = packInfo.applicationInfo.uid;
				app = map.get(uid);
				// filter applications which are not allowed to access the Internet
				if (app == null && PackageManager.PERMISSION_DENIED == pm.checkPermission(Manifest.permission.INTERNET, packInfo.packageName)) {
					continue;
				}
				// try to get the application label from our cache - getApplicationLabel() is horribly slow!!!!
				cachekey = "cache.label."+packInfo.packageName;
				name = prefs.getString(cachekey, "");
				if (name.length() == 0) {
					// get label and put on cache
					name = packInfo.applicationInfo.loadIcon(pm).toString();
					edit.putString(cachekey, name);
					changed = true;
				}
				if (app == null) {
					app = new DroidApp();
					app.icon = packInfo.applicationInfo.loadIcon(pm);
					app.uid = uid;
					app.name = packInfo.applicationInfo.loadLabel(pm).toString();
	...
	public static boolean hasRootAccess(Context ctx, boolean showErrors) {
		...
		if (showErrors) {
			alert(ctx, ctx.getString(R.string.root_err_tip));
		}
		return false;
	}
    /**
	...
	public static boolean assertBinaries(Context ctx, boolean showErrors) {
		...
		try {
			...
			if (changed) {
				LogUtils.jLog().e("Binary files installed!");
			}
		} catch (Exception e) {
